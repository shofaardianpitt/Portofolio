...
// @ts-ignore - ogl tidak memiliki tipe bawaan yang lengkap
import { Renderer, Triangle, Program, Mesh, Texture, OGLRenderingContext } from 'ogl';
...
class MorphEngine {
    ...
    items: MorphSliderItem[];
    getOptions: () => Partial<MorphSliderProps>;
    onIndexChange?: (index: number) => void;
    ...
    constructor(container: HTMLDivElement, { items, startIndex, reducedMotion, getOptions, onIndexChange, dprCap }: {
        items: MorphSliderItem[];
        startIndex: number;
        reducedMotion: boolean;
        getOptions: () => Partial<MorphSliderProps>;
        ...
        this.program = new Program(this.gl, {
            ...
            uniforms: {
                ...
                uProgress: { value: 0 },
                uDir: { value: 1 },
                uMode: { value: TRANSITIONS[opts.transition || 'melt'] ?? 0 },
                uIntensity: { value: opts.intensity ?? 0.55 },
                uScale: { value: opts.scale ?? 2.4 },
                uAberration: { value: opts.aberration ?? 0.35 },
                uDrift: { value: opts.drift ?? 0.4 },
                uTime: { value: 0 },
                uReduce: { value: reducedMotion ? 1 : 0 },
                uPointer: { value: [0.5, 0.5] },
                uOverlay: { value: hexToRgb(opts.overlayColor || '#000000') }
            }
        });
    ...
    syncOptions() {
        const opts = this.getOptions();
        this.program.uniforms.uMode.value = TRANSITIONS[opts.transition || 'melt'] ?? 0;
        this.program.uniforms.uIntensity.value = opts.intensity ?? 0.55;
        this.program.uniforms.uScale.value = opts.scale ?? 2.4;
        this.program.uniforms.uAberration.value = opts.aberration ?? 0.35;
        this.program.uniforms.uDrift.value = opts.drift ?? 0.4;
        this.program.uniforms.uOverlay.value = hexToRgb(opts.overlayColor || '#000000');
    }

    loop(t: number) {
        if (this.isDestroyed || !this.mesh || !this.renderer) return;
    ...
    goToTarget(targetIndex: number) {
        ...
        this.syncOptions();
        this.prepareTarget(targetIndex, dir);

        this.animating = true;
        this.announce(targetIndex);

        const opts = this.getOptions();
        const duration = this.reducedMotion ? Math.min(opts.duration || 1.1, 0.4) : (opts.duration || 1.1);
    ...
    next() {
        const nextIdx = this.wrap(this.current + 1);
        this.goToTarget(nextIdx);
    }

    prev() {
        const prevIdx = this.wrap(this.current - 1);
        this.goToTarget(prevIdx);
    }
...
export default function MorphSlider({
    ...
    const optsRef = useRef<Partial<MorphSliderProps> | undefined>(undefined);
    optsRef.current = {
        transition,
        duration,
        ease,
    ...
    useEffect(() => {
        if (!containerRef.current || !items || items.length === 0) return undefined;
    ...
    useEffect(() => {
        if (!autoplay || hovering || !items || items.length < 2) return undefined;
        const id = setTimeout(() => engineRef.current?.next(), Math.max(autoplayDelay, 1) * 1000);
        return () => clearTimeout(id);
    }, [autoplay, autoplayDelay, hovering, index, items]);

    useEffect(() => {
        const el = containerRef.current;
        if (!el || !items || items.length < 2) return undefined;
        let startX = 0;
        let width = 1;
        let active = false;
                ...
                <div className="morph-slider-indicators" role="tablist" aria-label="Slides">
                    {items.map((_, i) => (
                        <button
                            ...
                            onClick={() => {
                                engineRef.current?.goToTarget(i);
                            }}
                        />
                    ))}
                </div>
